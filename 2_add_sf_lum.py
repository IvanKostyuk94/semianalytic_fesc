...
def add_sf_quantities(df, sim_path, snap_num, z):
    ...
    for idx in df.index:
        ...
        relevant_stars_half_rad = select_stars_in_gas(
            stars_in_half_rad, gas_in_half_rad
        )
        ...
        stars_in_rad["ages"] = spectra._computeAgeFromFormationTime(
            stars_in_rad["Redshift"], stars_in_rad["GFM_StellarFormationTime"]
        )

        stars_in_half_rad["ages"] = spectra._computeAgeFromFormationTime(
            stars_in_half_rad["Redshift"], stars_in_half_rad["GFM_StellarFormationTime"]
        )

        if stars_in_rad["count"] < 10000:
            tot_ion_em_rad, tot_bol_lum_rad = compute_small_gal_em(
                stars_in_rad, specFac, idx
            )
            tot_ion_em_half_rad, tot_bol_lum_half_rad = compute_small_gal_em(
                stars_in_half_rad, specFac, idx
            )
        else:
            tot_ion_em_rad, tot_bol_lum_rad = compute_large_gal_em(
                stars_in_rad, specFac, idx
            )
            tot_ion_em_half_rad, tot_bol_lum_half_rad = compute_large_gal_em(
                stars_in_half_rad, specFac, idx
            )

        if relevant_stars_half_rad["count"] != 0:
            relevant_stars_rad["ages"] = spectra._computeAgeFromFormationTime(
                relevant_stars_rad["Redshift"],
                relevant_stars_rad["GFM_StellarFormationTime"],
            )

            relevant_stars_half_rad["ages"] = spectra._computeAgeFromFormationTime(
                relevant_stars_half_rad["Redshift"],
                relevant_stars_half_rad["GFM_StellarFormationTime"],
            )

            if relevant_stars_rad["count"] < 10000:
                ion_em_rad, bol_lum_rad = compute_small_gal_em(
                    relevant_stars_rad, specFac, idx
                )
                ion_em_half_rad, bol_lum_half_rad = compute_small_gal_em(
                    relevant_stars_half_rad, specFac, idx
                )
            else:
                ion_em_rad, bol_lum_rad = compute_large_gal_em(
                    relevant_stars_rad, specFac, idx
                )
                ion_em_half_rad, bol_lum_half_rad = compute_large_gal_em(
                    relevant_stars_half_rad, specFac, idx
                )

        else:
            ion_em_rad, bol_lum_rad = 0.0, 0.0
            ion_em_half_rad, bol_lum_half_rad = 0.0, 0.0
        ...
        metallicities_2r.append(
            np.sum(gas_in_rad["Z"] * gas_in_rad["Masses"])
            / np.sum(gas_in_rad["Masses"])
        )
        ...
        metallicities_r.append(
            np.sum(gas_in_half_rad["Z"] * gas_in_half_rad["Masses"])
            / np.sum(gas_in_half_rad["Masses"])
        )
    ...
    df["Tot_Bol_lum_2r"] = np.array(tot_bol_lums_2r) * L_sun_to_erg_s
    df["Tot_Ion_em_2r"] = np.array(tot_ion_ems_2r)

    df["Tot_Bol_lum_r"] = np.array(tot_bol_lums_r) * L_sun_to_erg_s
    df["Tot_Ion_em_r"] = np.array(tot_ion_ems_r)
    return
