...
def get_column_height_dens(df):
    ...
    df['Column_dens_r'] = df['M_gas_r']*df['Column_height_r']/df['V_r']/m_p.value
    df['Column_dens_2r'] = df['M_gas_2r']*df['Column_height_2r']/df['V_2r']/m_p.value
...
def gravitational_pressure(df):
    ...
    df['p_g_sf_r'] = m3_by_kg_to_cm3_by_g*np.pi/2*constants.G.value*df['Sigma_gas_sf_r']**2/df['f_g_sf_r']
    df['p_g_sf_2r'] = m3_by_kg_to_cm3_by_g*np.pi/2*constants.G.value*df['Sigma_gas_sf_2r']**2/df['f_g_sf_2r']
    return
...
def particle_dens(df):
    ...
    kg_to_g = 1e3
    df['n_gas_r'] = df['M_gas_r']/df['V_r']/(constants.m_p.value*kg_to_g)/mean_molecular_mass
    df['n_gas_2r'] = df['M_gas_2r']/df['V_2r']/(constants.m_p.value*kg_to_g)/mean_molecular_mass
    df['n_gas_sf_r'] = df['M_gas_sf_r']/df['V_sf_r']/(constants.m_p.value*kg_to_g)/mean_molecular_mass
    df['n_gas_sf_2r'] = df['M_gas_sf_2r']/df['V_sf_2r']/(constants.m_p.value*kg_to_g)/mean_molecular_mass
    return
...
def photon_to_gas(df):
    ...
    df['U_sf_r'] = df['Ion_flux_sf_r']/df['n_gas_sf_r']/constants.c.value
    df['U_sf_2r'] = df['Ion_flux_sf_2r']/df['n_gas_sf_2r']/constants.c.value
    return
...
def column_dens_stroemgren(df):
    ...
    df['Column_dens_stroemgren_sf_r'] = df['f_g_sf_r']*constants.c.value*meter_to_cm/case_B_param
    df['Column_dens_stroemgren_sf_2r'] = df['f_g_sf_2r']*constants.c.value*meter_to_cm/case_B_param
    return
...
def radiation_pressure(df):
    ...
    df['p_r_sf_r'] = (1-np.exp(-df['tau_d_sf_r']))*df['Bol_flux_sf_r']/(constants.c.value*meter_to_cm)
    df['p_r_sf_2r'] = (1-np.exp(-df['tau_d_sf_2r']))*df['Bol_flux_sf_2r']/(constants.c.value*meter_to_cm)
    return
    ...
    if mode == 'r':
        ...
        return (2*element['Column_height_r']
                * (element['p_r_sf_r']-element['p_g_sf_r'])
                / (element['Sigma_gas_sf_r']))**0.5
    elif mode == 'sf_2r':
        return (2*element['Column_height_2r']
                * (element['p_r_sf_2r']-element['p_g_sf_2r'])
                / (element['Sigma_gas_sf_2r']))**0.5
    else:
...
def add_outflow_vel(df):
    ...
    df['v_inf_sf_r'] = df.apply(lambda x: 0 if x['p_r_sf_r'] < x['p_g_sf_r']
                              else outflow_vel(x, mode='sf_r'), axis=1)
    df['v_inf_sf_2r'] = df.apply(lambda x: 0 if x['p_r_sf_2r'] < x['p_g_sf_2r']
                              else outflow_vel(x, mode='sf_2r'), axis=1)
    return
...
def critical_gas_fraction(element, mode='r'):
    if mode == 'r':
        ...
        return 6*(element['Dust_norm_sf_r'] * element['U1_sf_r']
               * ((1-element['N_ratio_sf_r']) / element['N_ratio_sf_r']))**(1/3)
    elif mode == 'sf_2r':
        return 6*(element['Dust_norm_sf_2r'] * element['U1_sf_2r']
               * ((1-element['N_ratio_sf_2r']) / element['N_ratio_sf_2r']))**(1/3)
    else:
...
def add_critical_gas_fraction(df):
    ...
    df['f_g_crit_sf_r'] = df.apply(lambda x: 0 if x['N_ratio_sf_r'] > 1
                                 else critical_gas_fraction(x, mode='sf_r'), axis=1)
    df['f_g_crit_sf_2r'] = df.apply(lambda x: 0 if x['N_ratio_sf_2r'] > 1
                                 else critical_gas_fraction(x, mode='sf_2r'), axis=1)
    return
...
def escape_fraction(element, mode):
    if mode == 'r':
        ...
        return np.exp(-element['N_red_sf_r']
                      * (1/element['Column_dens_stroemgren_sf_r']
                      + 1/element['N_d_sf_r']))
    elif mode == 'sf_2r':
        return np.exp(-element['N_red_sf_2r']
...
def f_esc(df):
    ...
    df['f_esc_sf_r'] = df.apply(lambda x: 0 if x['f_g_sf_r'] < x['f_g_crit_sf_r']
                              else escape_fraction(x, mode='sf_r'), axis=1)
    df['f_esc_sf_2r'] = df.apply(lambda x: 0 if x['f_g_sf_2r'] < x['f_g_crit_sf_2r']
                              else escape_fraction(x, mode='sf_2r'), axis=1)
    ...
    df['f_esc_sf_r'] = df.apply(lambda x: 1 if np.isnan(x['Column_dens_r'])
                              else x['f_esc_sf_r'], axis=1)
    df['f_esc_sf_2r'] = df.apply(lambda x: 1 if np.isnan(x['Column_dens_2r'])
                              else x['f_esc_sf_2r'], axis=1)
    return
